####################################################################################################
# Script to properly export all Failed Logic Apps Runs History for a specific Logic App Consumption
#
...
do {
    ...
    # Export only failed runs to .json files
    foreach ($run in $runs.value) {
        # Check if the run status is "Failed"
        if ($run.properties.status -eq "Failed") {
            ...
            $runId = "$logicAppName" + "_" + $run.name + "_" + $formattedStartTime + "_Failed"

...
{
    public static class FA_Monitoring_Top10Users
    {
        [FunctionName("FA_Monitoring_Top10Users")]
            ...
            ILogger log)
        {
            ...
            foreach (var user in groups)
            {
                ...
                JToken resourceGroup = user.User;

                rowType = rnd.Next(1, 3);

                string apiReportRow = @"
                <tr>
                    <td style='margin-left:10px; margin-top:5px; margin-bottom:5px;'>" + user.User + @"</td>
